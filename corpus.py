    ...
    with open(self.path) as ELM:
      for line in ELM:
        if len(line) > 1:
          ...
          sentence_corpus = self.splitNotSignAll(sentences=sentence, index=index,
            windownSplit=window, nAround=nAround)
          result.append(sentence_corpus)
          
          sentence_input = self.splitNotSignAll(sentences=sentence, index=index,
            windownSplit=window, nAround=n_around_pos_neg)

          if kinase in objLine[1]:
            nPositive += 1
            filePositive.write(sentence_input + "\n")
          else:
            negative.append(sentence_input)
